      ...
      <div className="bg-[#18181B] border border-[#3F3F46] rounded-lg overflow-hidden">
        ...
        <div className="divide-y divide-[#3F3F46]">
          {projects.map(project => (
            <div key={project.id} className="p-4 flex gap-4 items-center hover:bg-[#27272A] transition-colors">
              <div className="flex-1">
                <Link href={`/projects/${project.id}`} className="font-medium text-white hover:text-[#8B5CF6]">
                  {project.name}
                </Link>
                {project.description && (
                  <p className="text-sm text-[#A1A1AA] mt-1 line-clamp-1">{project.description}</p>
                )}
              </div>
              <div className="w-64 text-sm text-[#A1A1AA]">{project.client.name}</div>
          ...
          {projects.length === 0 && (
            <div className="p-8 text-center text-[#A1A1AA]">No projects found. Create a project to add briefs.</div>
          )}
        </div>
      </div>
    </div>
