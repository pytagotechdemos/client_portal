      ...
      <div className="bg-surface border border-border rounded-lg overflow-hidden">
        ...
        <div className="divide-y divide-[#3F3F46]">
          {changeRequests.map(cr => (
            <div key={cr.id} className="p-4 flex gap-4 items-center hover:bg-surface-hover transition-colors">
              <div className="flex-1">
                <p className="font-medium text-white">From {cr.requestedBy}</p>
                <p className="text-sm text-muted mt-1 line-clamp-1">{cr.description}</p>
              </div>
              <div className="w-64">
          ...
          {changeRequests.length === 0 && (
            <div className="p-8 text-center text-muted">No change requests found.</div>
          )}
        </div>
      </div>
    </div>
