  ...
  return (
    <div className="max-w-4xl">
      ...
      <div className="text-sm text-muted mb-4">
        <Link href="/projects" className="hover:text-white">Proyek</Link> &gt;{" "}
      ...
      <div className="flex justify-between items-start mb-8">
        <div>
          ...
          <div className="flex items-center gap-3 mt-2">
            ...
            <span className="text-muted text-sm">Tipe: {deliverable.type}</span>
            {deliverable.assignedTo && (
              <span className="text-muted text-sm bg-surface-hover px-2 py-1 rounded-md border border-border">
                Ditugaskan ke: <span className="text-white font-medium">{deliverable.assignedTo}</span>
              </span>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-8">
        <div className="col-span-2 space-y-6">
          <h3 className="text-xl font-bold text-white mb-4">Riwayat Versi</h3>
          ...
          <div className="space-y-4 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-[#3F3F46] before:to-transparent">
            {deliverable.versions.length === 0 ? (
              <p className="text-muted">Belum ada versi yang diunggah.</p>
            ) : (
                ...
                <div key={ver.id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                  ...
                  <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded-lg border border-border bg-surface">
                      <div className="flex justify-between items-start">
                        <div>
                          <p className="font-semibold text-white">Versi {ver.version}</p>
                          <p className="text-xs text-muted mb-2">
                            Diunggah oleh {ver.uploadedBy} pada {new Date(ver.uploadedAt).toLocaleString('id-ID')}
                          </p>

                          <div className="mt-2 space-y-3">
                            {ver.pmNotes && (
                              <p className="text-sm text-[#E2E8F0]">
                                <span className="font-medium text-[#94A3B8]">Catatan:</span> {ver.pmNotes}
                              </p>
                            )}

                            {ver.clientFeedback && (
                              <div className="bg-[#FEF2F2] border border-[#FECACA] rounded-md p-3 mt-3">
                                <p className="text-sm text-[#991B1B] font-medium mb-1">Umpan Balik Klien:</p>
                    ...
                    {ver.clientFeedback && (
                      <div className={`mt-3 p-2 rounded text-xs ${ver.clientAction === "APPROVED" ? "bg-[#022C22] border border-[#059669]" : "bg-[#450A0A] border border-[#B91C1C]"}`}>
                        <span className="font-bold block mb-1">{ver.clientAction === "APPROVED" ? "Disetujui" : "Revisi Diminta"}</span>
                        {ver.clientFeedback}
                      </div>
                    )}
                  </div>
        ...
        <div className="col-span-1">
          <div className="bg-surface border border-border rounded-lg p-5 sticky top-6">
            <h3 className="font-bold text-white mb-4">Unggah Versi Baru</h3>
            <form action={uploadVersion} className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-muted mb-1">Unggah File</label>
              ...
              <div>
                <label className="block text-xs font-medium text-muted mb-1">Tautan Eksternal (Figma, Dokumen, dll)</label>
              ...
              <div>
                <label className="block text-xs font-medium text-muted mb-1">Catatan untuk Klien (Opsional)</label>
