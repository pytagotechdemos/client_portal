  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      <BarChart data={data} margin={{ top: 5, right: 30, left: 0, bottom: 5 }}>
        ...
        <YAxis width={120} stroke="#A1A1AA" tick={{fill: '#A1A1AA'}} tickLine={false} axisLine={false} tickFormatter={(val) => `Rp ${Number(val).toLocaleString('id-ID')}`} />
        <Tooltip cursor={{fill: '#3F3F46', opacity: 0.4}} contentStyle={{ backgroundColor: '#18181B', border: '1px solid #3F3F46', borderRadius: '8px', color: '#fff' }} formatter={(val: any) => [`Rp ${Number(val || 0).toLocaleString('id-ID')}`, 'Pendapatan']} />
